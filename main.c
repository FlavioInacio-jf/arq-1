...
void decodeInstruction(uint8_t *executa, uint32_t registers[SIZE_MEMORY], uint8_t *mem8, uint32_t *mem32);
...
void decodeInstruction(uint8_t *executa, uint32_t registers[SIZE_MEMORY], uint8_t *mem8, uint32_t *mem32)
{
  ...
  case 0b011000:
    ...
    // Formatacao de saida em tela (deve mudar para o arquivo de saida)
    printf("0x%08X:\t%-25s\tR%u=MEM[0x%08X]=0x%02X\n", registers[29], instrucao, z, registers[x] + i, registers[z]);
    break;
  // l32
  case 0b011010:
    // Otendo operandos
  ...
  case 0b110111:
    ...
    // Formatacao da instrucao
    sprintf(instrucao, "bun %i", registers[28] & 0x3FFFFFF);
    // Formatacao de saida em tela (deve mudar para o arquivo de saida)
    printf("0x%08X:\t%-25s\tPC=0x%08X\n", pc, instrucao, registers[29] + 4);
    break;
  // int
  case 0b111111:
    // Parar a execucao
...
void decodeInstructions(uint32_t registers[SIZE_MEMORY], uint8_t *mem8, uint32_t *mem32)
{
  ...
  uint8_t executa = 1;
  while (executa)
    decodeInstruction(&executa, registers, mem8, mem32);
